...
export const CONTRIBUTOR_ROLES = [
  'المُحقِّق', 'المُراجِع', 'المُعتَني', 'المُصحِّح', 'ضبْط النصّ', 'المُخَرِّج',
  'المُتَرجِم', 'الإشراف العلميّ', 'تَقْرِيظ', 'تقديم',
]
...
const ROLE_GROUP: Record<string, string> = {
  'ضبْط النصّ': 'أصحاب ضبْط النصّ',
  'الإشراف العلميّ': 'المُشرِفون علميًّا',
  'تَقْرِيظ': 'أصحاب التقريظ',
  'تقديم': 'أصحاب التقديم',
}

/**
 * التأليفُ صفةٌ كسائر الصفات في العرض، وأعلاها جميعًا في الرتبة. وليس في
 * `CONTRIBUTOR_ROLES` لأنه لا يُدخَل من صفّ المشاركين بل من حقل المؤلِّف.
 */
export const AUTHOR_ROLE = 'المؤلِّف'

/**
 * ترتيبُ الصفات في العلوّ: التأليفُ فوقها جميعًا، ثم هي على ترتيبها في
 * `CONTRIBUTOR_ROLES`. وبه يُنعَت الرجلُ نعتًا مفردًا حين لا يسع المقامَ
 * تعدادُ صفاته — «تعديل بيانات المُحقِّق» لمن حقّق واعتنى.
 *
 * والعلوُّ يحكم اللفظَ المفرد وحده: لا يُسقط صفةً دونه، فالرجلُ يُعرض في
 * صفِّ كل صفةٍ سُجِّلت له، وتُقسَم أعمالُه بها كلِّها بلا خلط.
 */
export const ROLE_RANK = [AUTHOR_ROLE, ...CONTRIBUTOR_ROLES]

/**
 * ما يُنعَت به صاحبُ الصفة مفردًا، كما يقع بعد «تعديل بيانات …».
 * و«تَقْرِيظ» و«تقديم» مصدران لا يُنعَت بهما رجل، فيُقال: صاحبُهما.
 */
export const ROLE_PERSON: Record<string, string> = {
  [AUTHOR_ROLE]: 'المؤلِّف',
  'المُحقِّق':  'المُحقِّق',
  'المُراجِع':  'المُراجِع',
  'المُعتَني':  'المُعتَني',
  'المُصحِّح':  'المُصحِّح',
  'المُخَرِّج': 'المُخَرِّج',
  'المُتَرجِم': 'المُتَرجِم',
  // «ضبْط النصّ» مصدرٌ لا يُنعَت به رجل، و«الإشراف العلميّ» كذلك — غير أنّ
  // للثاني وصفًا يصحّ فيُعدَل إليه: المُشرِف علميًّا.
  'ضبْط النصّ':  'صاحب ضبْط النصّ',
  'الإشراف العلميّ': 'المُشرِف علميًّا',
  'تَقْرِيظ':   'صاحب التقريظ',
  'تقديم':     'صاحب التقديم',
}

/** نعتُ صاحب الصفة مفردًا، وما لا نعت له يبقى على لفظه */
export function rolePersonLabel(role: string): string {
  return ROLE_PERSON[role] ?? role
}

/**
 * النعتُ في موضع الإشارة، كما يقع بعد لامٍ: «لم تُكتب ترجمةٌ **لهذا
 * المؤلِّف** بعد»، و«**لصاحب التقريظ**».
 *
 * والفرقُ نحويّ لا اصطلاحيّ: «المؤلِّف» معرَّفٌ بأل فيُشار إليه بـ«هذا»،
 * و«صاحب التقريظ» مضافٌ لا تدخل عليه الإشارةُ على هذا الوجه.
 */
export function rolePersonRef(role: string): string {
  const label = rolePersonLabel(role)
  return label.startsWith('صاحب') ? label : `هذا ${label}`
}

/**
 * ما عمله الرجل بصفةٍ ما، مصدرًا مضافًا إليه: «تحقيقاته» لا «ما حقّقه».
 * المصدرُ أوجزُ وأشبهُ بلسان الفهارس.
 */
export const ROLE_WORKS: Record<string, string> = {
  [AUTHOR_ROLE]: 'مؤلَّفاته',
  'المُحقِّق':  'تحقيقاته',
  'المُراجِع':  'مراجعاته',
  'المُعتَني':  'اعتناءاته',
  'المُصحِّح':  'تصحيحاته',
  'المُخَرِّج': 'تخريجاته',
  'المُتَرجِم': 'ترجماته',
  // ليس لكلّ صفةٍ مصدرٌ يُضاف إليه: «ضبْطُ نصوصه» ثقيل، والصلةُ أخفُّ منه
  // وأبينُ — وهي لفظُ صاحب المكتبة نفسِه.
  'ضبْط النصّ':  'ما ضبطَ نصَّه',
  'الإشراف العلميّ': 'ما أشرفَ عليه',
  'تَقْرِيظ':   'تقريظاته',
  'تقديم':     'تقديماته',
}

/** عنوانُ رفِّ أعماله بهذه الصفة */
export function roleWorksLabel(role: string): string {
  return ROLE_WORKS[role] ?? `ما عمل فيه بصفة «${role}»`
}
 ...
 *
 * و`scope` نطاقُ ما أخرجَته من الكتاب، ويجري فيه ما يجري في نطاق المشارِك:
 * الفراغُ معناه أنها شرَيكةٌ في النشرة كلِّها — كدار ابن النفيس ودار التدوين
 * العربي على غلافٍ واحد — والمكتوبُ فيه يقسم الكتاب بينها وبين غيرها:
 * «١-٣» لدار القلم، و«٤-٢٠» لكنوز إشبيليا.
 */
...
/**
 * هيئةُ النشرة التي بين يديك.
 *
 * والأصلُ لا يُذكر: النُّسَخُ أصولٌ عند الناس كلِّهم حتى يُقال غيرُ ذلك، فلا
 * تُعلَّم بشيء — وإنما الخبرُ فيما فارق الأصل. ولذلك الفراغُ ههنا هو الأصل،
 * والقائمةُ ما سواه.
 *
 * والمصوَّرةُ غيرُ إعادة الصفّ: تلك تصويرُ النشرة كما هي لم يُمسّ فيها حرف،
 * وهذه نصُّها يُنسخ ثم يُعاد صفُّه في قالبٍ جديد. وكلتاهما ليست نشرةً
 * مستقلَّةً عن أصلها، والمصوَّرةُ أقلُّ قيمةً أثريّةً من الأصل.
 */
export const ISSUE_KINDS = ['مصوَّرة', 'إعادة صفّ']

/** شرحُ كلّ هيئةٍ، يُعرض تحت اختيارها فلا تلتبس بأختها */
export const ISSUE_HINTS: Record<string, string> = {
  'مصوَّرة': 'صُوِّرت عن نشرةٍ سابقة كما هي، لم يُغيَّر فيها حرف',
  'إعادة صفّ': 'نصُّ نشرةٍ سابقة أُعيد صفُّه في قالبٍ جديد، وليست نشرةً مستقلَّة',
}

/** ما يُسأل عنه في النموذج بعد اختيار الهيئة: «من صوَّرها»، «من أعاد صفَّها» */
export const ISSUE_BY_LABEL: Record<string, string> = {
  'مصوَّرة': 'من صوَّرها',
  'إعادة صفّ': 'من أعاد صفَّها',
}
...
export interface Book {
  ...
  publisher_id: string | null
  publisher: string
  /** نطاقُ ما أخرجَته الدارُ الأولى، والفراغُ: النشرةُ كلُّها */
  publisher_scope: string
  /** دُورٌ شارَكت في إخراج النشرة، الأولى في `publisher_id` وهؤلاء بعدها */
  co_publishers: CoPublisher[]
  place: string
  year: number | null
   ...
   * سنةً بين مجلَّده الأول وآخره. والصفرُ فيها: لم تُعرف سنةُ هذا المجلَّد.
   */
  volume_years: number[]
  /** ما نقص من مجلَّدات الطبعة، لكلٍّ رقمُه وسببُ فقده */
  missing_volumes: MissingVolume[]
  /** هيئةُ هذه النشرة: فراغٌ للأصل، وإلّا فمن `ISSUE_KINDS` */
  issue_kind: string
  /** من صوَّرها أو أعاد صفَّها */
  issue_by: string
  /** سنةُ التصوير أو إعادة الصفّ، بحساب `year_era` نفسه */
   ...
   * لكلٍّ محقِّقُه ودارُه ومجلَّداتُه — وليسا عنوانَين اثنين، فتُنسَب الدُّونى
...
export const META_DEFS: { label: string; key: string }[] = [
  ...
  { label: 'نُسَخُه في المكتبة',  key: 'copies' },
...
export const BOOK_PRIVACY_FIELDS: { label: string; key: string }[] = [
  ...
  { label: 'نشراتُه الأخرى',      key: 'otherEditions' },
...
export const MATNS_COUNT: CountForms = {
  none: 'لا متن', one: 'مَتْنٌ واحد', two: 'مَتْنان', few: 'مُتُونٍ', many: 'مَتْنًا',
}

export const ISSUES_COUNT: CountForms = {
  none: 'لا نشرة', one: 'نشرةٌ واحدة', two: 'نشرتان', few: 'نشَراتٍ', many: 'نشرةً',
}
