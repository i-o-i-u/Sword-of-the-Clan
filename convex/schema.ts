...
/**
 * نوعُ الفائدة: **نصٌّ حرّ لا اتّحادُ ألفاظٍ مغلق**.
 *
 * كان اتّحادًا مغلقًا، فلم يكن لصاحب المكتبة أن يزيد نوعًا ولا أن يُعدِّل
 * اسمَ نوع — وتلك أبوابُ كنّاشه هو، لا تُملى عليه. فالأنواعُ اليوم جدولٌ
 * قائمٌ (`perk_kinds`) لكلّ نوعٍ فيه أيقونتُه، وهذا الحقلُ يحمل اسمَ ما
 * اختِير منها.
 *
 * وتعديلُ اسم النوع يُزامَن على فوائده في `catalog.savePerkKinds` — كما
 * يُزامَن اسمُ المؤلِّف على كتبه — وإغفالُه يترك فوائدَ بنوعٍ لا وجود له.
 */
export const perkKind = v.string()

/**
 * مصدرُ الفائدة حين لا يكون من كتب المكتبة: ما قُرئ في مكتبةٍ عامّة، أو في
...
export const perkSource = v.object({
  title: v.string(),
  author: v.string(),
  /** وفاةُ مؤلِّفه إن عُرفت، تُكتب نصًّا: «ت ٢٩١ هـ» */
...
/**
 * عنوانٌ طُبِع مع الكتاب أو فيه، وهو كتابٌ لا يستقلّ ماديًّا بنفسه: يشترك
 * مع الضامِّ في الدار والسنة والطبعة والمجلَّدات والموضع من الرفّ، فلا
 * يُعاد شيءٌ من ذلك ههنا. وإنما يُكتب ما ينفرد به: عنوانُه ومؤلِّفُه وذوو
 * صفاته وتصنيفُه وموضعُه من الكتاب.
 *
 * وهو مع ذلك **يُعدّ كتابًا** في أعداد المكتبة: مجموعةٌ طُبع فيها عشرون
 * كتابًا زادت المكتبةَ عشرين — ولا يزيدها مجلَّدًا واحدًا ولا صفحةً واحدة،
 * إذ ليس له من الورق شيءٌ على حِدَة.
 */
export const withinTitle = v.object({
  title: v.string(),
  ...
  /** موضعُه من الكتاب الضامّ: «ص٣٠-٦٠»، «ج١، ص٥» */
...
export const bookFields = {
  ...
  // ما طُبِع مع هذا الكتاب أو فيه من العناوين. وهما وجهان لا وجه:
  //   • **مطبوعٌ معه**: للنشرة كتابٌ رئيس هو المقصود بالطبع، ثم أُلحق به
  //     غيرُه تكملةً وتتميمًا. والسجلُّ سجلُّ ذلك الرئيس.
  //   • **مطبوعٌ فيه**: العنوانُ اسمُ مجموعةٍ لا اسمُ كتاب — «برنامج مهمّات
  //     العلم» — فليس لها مؤلِّف وإنما مَن أشرف عليها، ولا تُعدّ هي كتابًا:
  //     المعدودُ ما طُبع فيها. وذلك ما يُعلَّم بـ`is_collection`.
  is_collection: v.optional(v.boolean()),
  within_titles: v.optional(v.array(withinTitle)),
  // وهذان من عهدٍ كان الكتابُ المضموم فيه سجلًّا قائمًا برأسه يشير إلى ضامِّه.
  // بقيا لِما فُهرس قبل التحويل، ويطويهما `maintenance.foldWithinBooks`.
...
export default defineSchema({
  ...
  perks: defineTable({
    ...
    /** نوعُها الأوّل. باقٍ لما فُهرس قبل أن تصير الأنواعُ قائمة. */
    kind: v.optional(perkKind),
    /**
     * أنواعُها. الفائدةُ تكون تحريرًا وتعقُّبًا معًا فلا تُحبَس في واحد،
     * وأسماؤها من جدول `perk_kinds`.
     */
    kinds: v.optional(v.array(v.string())),
    title: v.string(),
    text: v.string(),
    /** النصُّ منسَّقًا. و`text` مجرَّدُه، عليه يقوم البحثُ والعزو. */
    text_html: v.optional(v.string()),
    /** هوامشُ النصّ: لكلٍّ مِسماكُه في `text_html` ونصُّه ههنا */
    footnotes: v.optional(v.array(v.object({ id: v.string(), text: v.string() }))),
    page: v.string(),

    /** المجلَّد الذي فيه الموضع، حين يكون الكتابُ مجلَّدات */
    volume: v.optional(v.string()),
    /** بابُها الأوّل وفرعُه. باقيان لما فُهرس قبل أن تصير الأبوابُ قوائم. */
    category: v.optional(v.string()),
    sub_category: v.optional(v.string()),
    /**
     * تصنيفاتُها وفروعُها، من جدولَي `perk_categories`. **وهي منفصلةٌ عن
     * تصنيفات المكتبة انفصالًا تامًّا**: تلك تصنيفُ كتابٍ في رفّ، وهذه بابٌ
     * من أبواب العلم تُنسب إليه الفائدة.
     */
    categories: v.optional(v.array(v.string())),
    sub_categories: v.optional(v.array(v.string())),
    /** الكرّاسات التي أُدخلت فيها. معرّفاتٌ نصًّا لا `v.id`: كرّاسةٌ حُذفت
     * لا تُفشل تحقُّقَ المخطّط على الفائدة، وإنما تسقط عند القراءة. */
    notebook_ids: v.optional(v.array(v.string())),
     ...
     * وهي تُعلَّم من صفحة الفائدة بعد قيدها لا من نموذجها: الحكمُ على
     * النفاسة إنما يكون بعد النظر في المقيَّد، فلا يُسأل عنه ساعةَ الكتابة.
     */
    rating: v.optional(v.number()),
    /** كرّاستُها الأولى. باقيةٌ لما فُهرس قبل أن تصير الكرّاساتُ جدولًا. */
  ...
  /**
   * أنواعُ الفوائد: تحريرٌ وتعقُّبٌ ونقلٌ ونحوها. جدولٌ لا قائمةٌ في
   * الإعدادات، لأن لكلّ نوعٍ أيقونةً يختارها صاحبُ المكتبة من مكتبة
   * الأيقونات (`src/lib/icons.tsx`)، والأيقونةُ خبرٌ عن النوع لا زينة.
   */
  perk_kinds: defineTable({
    name: v.string(),
    /** مفتاحُ أيقونتها من مكتبة الأيقونات */
    icon: v.optional(v.string()),
    /** شرحُ النوع، يُعرض تحت اختياره في النموذج */
    hint: v.optional(v.string()),
    /** موضعُه من الصفّ، كما رتّبه صاحبُ المكتبة */
    order: v.optional(v.number()),
  }).index('by_name', ['name']),

  /**
   * تصنيفاتُ الفوائد: أبوابُ العلم التي تُنسب إليها الفائدة — القرآن
   * وعلومه، والأنساب، والسيرة… **ولا علاقةَ لها بتصنيفات المكتبة**: تلك
   * تُصنَّف بها الكتبُ على الأرفف، وهذه تُصنَّف بها الفوائد.
   *
   * والفرعُ صفٌّ فيه `parent` اسمُ أصله، كما في `categories` سواءً.
   */
  perk_categories: defineTable({
    name: v.string(),
  ...
  /**
   * الأعلامُ المُسجَّلون: من يُذكر في الفوائد. كانوا أسماءً تُكتب في كل
   * فائدة فيفترق الاسمُ الواحد بوجهين، فصاروا سجلًّا يُختار منه.
   */
  perk_figures: defineTable({
    name: v.string(),
    /** وفاتُه إن عُرفت، تُكتب نصًّا: «ت ٢٩١ هـ» */
    death: v.optional(v.string()),
    note: v.optional(v.string()),
  }).index('by_name', ['name']),

  /**
   * الكرّاسات: مسائلُ يُجمع لها المتفرِّق من الفوائد. ولها بابُها من صفحة
   * الفوائد، **ومن صفحة كل كرّاسةٍ تُضاف الفوائدُ الداخلة فيها** — لا من
   * نموذج الفائدة: الكرّاسةُ تقوم بعد أن يجتمع لها شيء.
   */
  perk_notebooks: defineTable({
    name: v.string(),
    note: v.optional(v.string()),
    icon: v.optional(v.string()),
  }).index('by_name', ['name']),
